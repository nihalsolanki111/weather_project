...
def data_get():
    ...
    response = requests.get(f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid=9f82136d1116521b6da50e9d97f0ac38')
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            w2_label_value.config(text=str(data["main"]["temp"] - 273.15))  # Convert Kelvin to Celsius
            w3_label_value.config(text=data["main"]["pressure"])   
            w4_label_value.config(text=data["main"]["humidity"])
 
        except KeyError:
            w_label_value.config(text="Data not available")
            w1_label_value.config(text="Data not available")
            w2_label_value.config(text="Data not available")
            w3_label_value.config(text="Data not available")
            w4_label_value.config(text="Data not available")

    else:
        w_label_value.config(text="Error fetching data")
        w1_label_value.config(text="Error fetching data")
        w2_label_value.config(text="Error fetching data")
        w3_label_value.config(text="Error fetching data")
        w4_label_value.config(text="Error fetching data")
...
name_list = ["Andhra Pradesh", "Arunachal Pradesh", "Assam", "Bihar", "Chhattisgarh", "Goa", "Gujarat",
             ...
             "Daman and Diu", "Lakshadweep", "National Capital Territory of Delhi", "Puducherry","bhavnagar"]
...
w4_label = Label(root, text="Humidity", font=("futura", 12, "bold"))
w4_label.place(x=150, y=440, height=40, width=210)
...
w4_label_value = Label(root, font=("futura", 12, "bold"))
w4_label_value.place(x=450, y=440, height=40, width=210)
